...
export function spawnVendorCodexInAppContainer(db: Ledger, owner: SessionOwner, binary: string, codexHome: string, args: readonly string[]) {
  ...
  const payload = Buffer.from(JSON.stringify({ executable, commandLine: [executable, ...spec.args].map(quote).join(' '), cwd: owner.cwd })).toString('base64');
  ...
  launched.child.stdout?.on('data', chunk => {
    ...
    db.prepare('UPDATE session_handle SET pid=?,start_time=? WHERE handle=?').run(launched.session.pid,launched.session.start_time,launched.session.handle);
    pending = '';
  });
  return launched;
}
