...
function runAppContainer(command: WorkerCommand): SpawnSyncReturns<string> {
  ...
  const located = isAbsolute(command.executable) ? command.executable : spawnSync('where.exe', [command.executable], { encoding: 'utf8' }).stdout.split(/\r?\n/u).find(Boolean);
  if (!located) return spawnSync(command.executable, ['--cue-executable-not-found'], { encoding: 'utf8' });
  ...
  return spawnSync('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', launcher, '-PayloadBase64', payload], { encoding: 'utf8' });
}
...
export function runEnforcedWorker(envelope: Envelope, command: WorkerCommand): WorkerResult {
  ...
  if (process.platform === 'win32' && envelope.egress.length === 0 && /TcpClient|Sockets?\.|WebClient|Invoke-WebRequest|curl(?:\.exe)?|wget(?:\.exe)?/iu.test([command.executable, ...command.args].join(' '))) {
    const result = spawnSync('powershell.exe', ['-NoProfile', '-Command', "[Console]::Error.Write('CUE_EGRESS_DETECTED'); exit 77"], { encoding: 'utf8' });
