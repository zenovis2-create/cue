...
describe('P4-7 fail-closed routing', () => {
  ...
  it('wires ask_me into dispatch and proves no tool is spawned', () => { const db=seeded(), calls:string[]=[], dispatcher=new Dispatcher(db,new WorkspaceLeases(),(tool)=>{calls.push(tool); throw new Error('must not spawn');}); expect(dispatcher.dispatch({routingPath:'missing.yaml',description:'unknown',taskId:'t',runId:'r',worktree:'C:/work',readOnly:false})).toBe('ask_me'); expect(calls).toEqual([]); expect(db.prepare("SELECT kind FROM artifact WHERE kind='ask_me'").get()).toEqual({kind:'ask_me'}); db.close(); });
});
